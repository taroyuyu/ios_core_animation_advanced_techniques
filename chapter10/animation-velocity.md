## 动画速度
动画实际上就是一段时间内的变化，这意味着变化必须以特定的速率或速度发生。速率由以下公式计算而来：

    velocity = change / time

这里的*change*可以指的是一个物体移动的距离，*time*指的是动画的持续时长。这对于涉及位移的动画(如位置或边界属性的动画)来说更容易可视化，但它同样适用于任何动画属性(如颜色或不透明度)。

上面的等式假设了速度在整个动画过程中都是恒定不变的（就如同第八章“显式动画”的情况），对于**这种恒定速度的动画我们称之为“*线性步调*”，而且从技术的角度而言这也是实现动画最简单的方式，但也是*完全不真实*的一种效果**。

考虑一辆短距离行驶的汽车。它不会从60英里每小时开始，开到目的地，然后立即下降到0英里每小时。首先，这将需要无限的加速能力(即使是最好的跑车也不能在0秒内从0加速到60英里)，其次，这将杀死所有的乘客。在现实中，它会慢慢加速到全速，然后当接近目的地时，它会开始减速，直到最后缓慢地停下来。

如果重物落在坚硬的表面上呢?它会从静止开始，然后继续加速直到它撞上表面，这时它会突然停止(可能伴随着一声巨响，因为它积累的动能变成了声音)。

现实世界中的每一个物体在移动时都会加速或减速。那么，如何在动画中实现这种加速呢?一种选择是使用物理引擎来真实地模拟我们的动画对象的摩擦和动量，但这对于大多数目的来说都是多余的。对于一个动画用户界面，我们只是想要一些时间方程，让我们的图层像真实世界的物体一样移动，但这并没有复杂到难以计算。这些类型的方程的名称是*Easing函数*，幸运的是Core Animation自带了一堆现成的标准函数。
...
那么该如何使用*Easing方程式*呢？首先需要设置`CAAnimation`的`timingFunction`属性，该属性是`CAMediaTimingFunction`类的一个对象。如果想改变*隐式动画*的`timingFunction`，同样也可以使用`CATransaction`的`+setAnimationTimingFunction:`方法。
...
**`CAKeyframeAnimation`有一个`NSArray`类型的`timingFunctions`属性，我们可以用它来对每次动画的步骤指定不同的*Easing函数*。但是指定函数的个数一定要等于`values`数组的元素个数*减一*，因为它是描述每一帧之间动画速度的函数**。
