...
&nbsp;&nbsp;&nbsp;&nbsp;当涉及到性能时，实际绘制图像所花费的时间通常不是限制因素。图片会消耗大量的内存，同时将应用程序需要显示的所有图片都加载到内存中可能是不切实际的，这意味着你需要在应用程序运行时定期加载和移除图片。

&nbsp;&nbsp;&nbsp;&nbsp;图片文件的加载速度不仅受到CPU的限制，还受到IO(输入/输出)延迟的限制。iOS设备中的闪存已经比传统硬盘快很多了，但仍然比RAM慢将近200倍左右，这就需要很小心地控制图像加载方式，以避免明显的延迟。

&nbsp;&nbsp;&nbsp;&nbsp;只要有可能，**尽量在应用程序生命周期中不显眼的时间加载图像**，例如启动，或者在屏幕切换的过程中。按下按钮和按钮响应事件之间最大的延迟大概是200ms，这比动画每一帧切换的16ms小得多。你可以在程序首次启动的时候加载图片，但是**如果20秒内无法启动程序的话，iOS watchdog计时器就会终止你的应用（而且如果启动大于2，3秒的话用户就会抱怨了）**。

&nbsp;&nbsp;&nbsp;&nbsp;有些时候，提前加载所有的东西并不明智。比如说包含上千张图片的图片轮播：用户希望能够能够平滑快速翻动图片，所以就不可能提前预加载所有图片；那样会消耗太多的时间和内存。

&nbsp;&nbsp;&nbsp;&nbsp;有时候图片也需要从远程网络连接中下载，这将会比从磁盘加载要消耗更多的时间，甚至可能由于连接问题而加载失败（在几秒钟尝试之后）。你不能在主线程上进行网络加载，并期望用户在屏幕冻结的情况下等待它。你需要使用一个后台线程。

### 线程加载

&nbsp;&nbsp;&nbsp;&nbsp;在第12章“性能调优”我们的联系人列表例子中，图片都非常小，所以可以在主线程同步加载。但是对于大图来说，这样做就不太合适了，因为加载会消耗很长时间，造成滑动的不流畅。滑动动画会在主线程的run loop中更新，所以会有更多运行在*渲染服务进程*中CPU相关的性能问题。

&nbsp;&nbsp;&nbsp;&nbsp;清单14.1显示了一个通过`UICollectionView`实现的基础的图片轮播器。图片在主线程中`-collectionView:cellForItemAtIndexPath:`方法中同步加载（见图14.1）。

清单14.1 使用`UICollectionView`实现的图片轮播器
...
&nbsp;&nbsp;&nbsp;&nbsp;传送器中的图片尺寸为800x600像素的PNG，对iPhone5来说，1/60秒要加载大概700KB左右的图片。当轮播器滚动的时候，图片也在实时加载，于是（预期中的）卡动就发生了。时间分析工具（图14.2）显示了很多时间都消耗在了`UIImage`的`+imageWithContentsOfFile:`方法中了。很明显，图片加载造成了瓶颈。
...
&nbsp;&nbsp;&nbsp;&nbsp;加载与解压所花费的相对CPU时间将取决于图像格式。对于PNG图像，加载时间比jpeg要长，因为文件的比例更大，但解压速度相对较快，特别是当Xcode使用最优设置对项目中包含的任何PNG进行重新压缩以实现快速解码时。JPEG图像更小，加载更快，但是解压缩步骤更昂贵，因为JPEG解压缩算法比PNG中使用的基于zip的算法更复杂。

&nbsp;&nbsp;&nbsp;&nbsp;当你加载一个图像时，iOS通常会延迟解压，以节省内存。当您真正尝试绘制图像时，这可能会导致性能中断，因为必须在绘制点(通常是最糟糕的时间)解压。
...
&nbsp;&nbsp;&nbsp;&nbsp;最后一种方式就是使用UIKit加载图片，但是需要立即将它绘制到`CGContext`中去。图像必须在绘制之前解压缩，因此这会迫使解压缩立即进行。这样做的好处是，绘图可以在后台线程上完成(像加载本身)，所以它不需要阻塞UI。
